...
public class Input
{
    ...
    public Input(string modFolder, string configDirectory)
    {
        ModFolder = modFolder;
        ...
        {
            var oldPath = Path.Combine(ModFolder, controllerFile);
            var newPath = Path.Combine(ConfigFolder, controllerFile);
        ...
        for (int x = 0; x < Mappings.ControllerFiles.Length; x++)
            Controllers[x] = new VirtualController(Path.Combine(ConfigFolder, Mappings.ControllerFiles[x]));
    }
    ...
    public void Configure()
    {
        ...
        for (int x = 0; x < input.Length; x++)
            input[x] = new ConfiguratorInput($"Controller {x}", Path.Combine(ConfigFolder, Mappings.ControllerFiles[x]), new ObservableCollection<MappingEntry>(Mappings.Entries));
